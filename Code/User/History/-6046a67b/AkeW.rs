    ...
    fn scan_token(&mut self) -> Result<Option<(TokenType, String)>, String> {
        ...
        match c {
            ...
            '%' => {
                ...
                else if self.match_char('d') {
                    Ok(Some((TokenType::IntFormatter, "%d".to_string())))
                }
                else if self.match_char('f') {
                    Ok(Some((TokenType::FloatFormatter, "%f".to_string())))
                }
                else if self.match_char('c') {
                    Ok(Some((TokenType::CharFormatter, "%c".to_string())))
                }
                else if self.match_char('s') {
                    Ok(Some((TokenType::StringFormatter, "%s".to_string())))
                }
                else if self.match_char('b') {
                    Ok(Some((TokenType::BoolFormatter, "%b".to_string())))
                }
                else {
